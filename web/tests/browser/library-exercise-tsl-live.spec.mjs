...
test('library-exercise-tsl-live', async ({ launch, check }) => {
  // Skipped unless asked for, and skipped rather than deleted from the suite: it needs a core on
  // :8080 with auth disabled and the tiivad container registered, which CI has no way to provide.
  // It stays a local verification tool — `HARNESS_LIVE=1 npx playwright test tsl-live` — and being
  // listed as skipped is how anyone reading the run knows it exists at all.
  test.skip(!process.env.HARNESS_LIVE, 'needs a real core on :8080 — set HARNESS_LIVE=1')

  const { page, shot, close } = await launch({ role: 'teacher,admin', shotPrefix: 'lib-ex-tsl-live-' })
