...
test('nav-bug-dashboard', async ({ launch, check }) => {
  ...
  // --- shown to an admin when configured ------------------------------------------------------------
  {
    ...
    for (const item of ['System messages', 'Reported bugs', 'Lahendus user admin', 'Operating info']) {
    ...
    check(
      ...
      ['System messages', 'Reported bugs', 'Lahendus user admin', 'Operating info'].every((i) =>
        lastFour.includes(i),
      ),
    )
    await r.close()
