 ...
 *
 * The example tests in `markdown-actions.test.mjs` pin down what each command *should* produce. These
...
test('markdown-invariants', () => {
  ...
  console.log(`\n  ${checked} property checks across ${DOCS.length} documents`)
  expect(failures.slice(0, 5)).toEqual([])

  // The count is asserted too, because a loop that stops iterating is the failure this file cannot
  // otherwise see: zero failures out of zero checks is indistinguishable from a clean run.
  // 11,903 today across 15 documents. A floor of 1000 left an 11x margin, so a generator that
  // produced one document instead of fifteen would still have sailed through.
  expect(checked).toBeGreaterThanOrEqual(11_000)
})
