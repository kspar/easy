...
//   cd web/dev-harness && node scripts/system-messages.mjs
...
async function boot(initial) {
  const { browser, page, shot } = await launch({ shotPrefix: 'sysmsg-' })
...
{
  const { browser, page, shot } = await boot([URGENT, INFO])
...
{
  const { browser, page, shot, state } = await boot([])
