...
//   cd web/dev-harness && node scripts/web-update-banner.mjs
//
// The exact comparison rules — same commit, rollbacks, `unknown`, malformed bodies — are pinned in
// unit/web-version.mjs, which needs no browser. What can only be checked here is the wiring: that
...
async function boot({ stamp, shotPrefix = 'webupdate-' } = {}) {
  const { browser, page, shot } = await launch({ shotPrefix })
  const state = { stamp }
  ...
  return { browser, page, shot, state }
}
...
{
  const { browser, page, shot } = await boot({ stamp: DEPLOYED })
...
{
  const { browser, page } = await boot({ stamp: DEPLOYED })
...
{
  const { browser, page, shot, state } = await boot({ stamp: DEPLOYED })
  await waitUntil(async () => (await alerts(page).count()) > 0)
...
{
  const { browser, page } = await boot({ stamp: 'html', shotPrefix: 'webupdate-quiet-' })
...
{
  const { browser, page } = await boot({ stamp: null, shotPrefix: 'webupdate-quiet-' })
  await page.waitForTimeout(1500)
...

{
  ...
  const { browser, page } = await boot({ stamp: { ...DEPLOYED, commit: 'unknown' }, shotPrefix: 'webupdate-quiet-' })
  await page.waitForTimeout(1500)
