...
class TeacherReadSubmissionSummariesController {
    ...
    fun controller(@PathVariable("courseId") courseIdString: String,
        ...
        return selectTeacherSubmissionSummaries(
            caller, courseId, courseExId, groupId,
            queryWords, orderBy, order, offsetStr?.toLongOrNull(), limitStr?.toIntOrNull()
        )
    }

    private fun selectTeacherSubmissionSummaries(
        caller: EasyUser, courseId: Long, courseExId: Long, groupId: Long?,
        queryWords: List<String>, orderBy: OrderBy, order: SortOrder,
        offset: Long?, limit: Int?
    ): Resp {
        return transaction {
            ...
            val subQuery = (
                    Join(
                        StudentCourseAccess leftJoin (StudentCourseGroup innerJoin CourseGroup), CourseExercise,
                        onColumn = StudentCourseAccess.course, otherColumn = CourseExercise.course
                    ) innerJoin
                ...
                .orderBy(
                    distinctStudentId to SortOrder.ASC,
