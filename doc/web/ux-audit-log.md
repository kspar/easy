...
| S5b | Large monitor, with data | **done `4a2d2627`** (2026-09-20), fixed the same day | EZ-1915, EZ-1916, EZ-1917, EZ-1918, EZ-1919 (all under EZ-1527) | S5 drove the teacher surfaces empty, so it could say the cap exists but not what it costs. With 35 students × 24 exercises and a 37-line solution, at 1920×1080 and 2560×1440: the grading editor is **639px on either monitor and clips the longest line by 445px**; the grade table shows **7 of 24** exercise columns with all 24 titles truncated; its `stickyHeader` sits at **−468px** after a scroll, because the container it sticks to only scrolls sideways; the grade form is below the fold at both sizes (column ends at 1769px of 1080). Five decisions taken by kspar the same day — per-route width, capped pages left-aligned, task text `min(40%, 720px)`, grade form in a third column, two-line table headers — and implemented: editor 1011–1480px, all 24 columns at 2560 and 17 at 1920, header holds, grade form beside the code. The lesson for the next sweep is S5's own: **an empty fixture cannot show a layout's cost** — and the suite's 1100px default viewport sits below `lg`, which is how none of this ever reached a spec. `large-monitor-layout.spec.mjs` is the gate now; `HARNESS_VIEWPORT=2560x1440` re-runs any spec wide |
