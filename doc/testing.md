...
generated from core's Kotlin by reflection and committed — 123 endpoints, 196 types, 188 nullable
fields, 32 enum-typed fields, as `bin/testcounts` reports them. A plain `@Test` regenerates it and
fails if the committed file differs.

Those four figures moved to the script for the reason this section's own opening argues: the file is
regenerated by a test, so only the prose describing it could rot, and it had — the types and nullable
counts read 194 and 184 here while the file said 196 and 188, drift nobody would notice by reading.
