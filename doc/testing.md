...
| | Size | Tests |
| --- | --- | --- |
| **core** | 118 `@RestController` classes (124 endpoints) | 37 test files, **199 tests, all running** |
...
199 is what the runner reports, not a count of `@Test`. The two differ now that some tests are
...
**All 199 core tests now run, in CI and on a laptop, with no setup** — 32 when this started, of which 25 ran. EZ-1715 landed on 2026-08-15,
...
generated from core's Kotlin by reflection and committed. A plain `@Test` regenerates it and fails if
the committed file differs. **`bin/testcounts` prints its size** — endpoints, types, nullable fields,
enum-typed fields — and no figure for it appears here on purpose.

That last sentence was learned twice in one day. The figures used to be inline and read "123
endpoints, 194 types, 184 nullable fields" while the file said 196 and 188 — drift nobody would catch
by reading. Corrected, moved to the script, and *the corrected copy left inline as well* — where the
enum count went stale within the hour, when EZ-1777's follow-up deleted three enum fields. So the
rule is not "measure it once more carefully". It is that **a number in prose is a cache with no
invalidation**, and the only fix that holds is not writing it down twice.
...
it was a trap rather than a break.

**Fixed 2026-08-21, twice.** First as a Kotlin enum on both requests and the response, uppercase on
the wire like every other enum here, with changeset `210826-3` normalising the existing rows. Then,
an hour later, by **deleting the field and its column** in `210826-4` — because the answer to "what
should this validate?" turned out to be that it carried nothing: the editor computed it as
`suggestedCode ? 'suggestion' : 'comment'` on every save, and `suggested_code` says the same thing
and cannot contradict itself. The check found a real disagreement; what it could not tell us is
whether the field should exist at all, and that question is the one that mattered.

Either way the waiver in `web/tests/api-types-baseline.json` had to go — that file fails when a
waiver stops firing, so the exemption could not outlive the exemption. `waivers` is now empty, which
is the mechanism working rather than a gap.
