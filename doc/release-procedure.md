...
| v4.0 (unreleased) | `easy.core.youtrack.*` added — six keys plus a secret (EZ-1786) | **Required on every environment**, or the context won't start: `enabled`, `base-url`, `project-id`, `visibility-group-id`, `issue-type-id`, `retry-cron`. EZ's real ids, confirmed 2026-08-23: project `0-0`, group `542-0` (EZ Team), type `81-29` (`User-submitted issue`). `issue-type-id` may be left blank, which files issues with the project's default type. The ansible-managed environments get them from `core_config`; only a hand-maintained `application.yaml` needs editing. Setting `enabled: false` and leaving the two ids empty is a complete, working configuration — reports are stored in `bug_report` and emailed to the sys address either way, and forwarding is what the flag turns off. To actually file issues, add `easy.core.youtrack.token` to **`secrets.yaml`, never to `application.yaml`** (the credential grep in `core_config` fails the play if a key matching `token` appears in the managed file), and fill in the two ids — which are YouTrack's internal ids, not `EZ` and not `EZ Team`. See `doc/bug-reporting.md` for the two `curl`s that print them and for why every filed issue is restricted to one group. A blank id or a `CHANGEME` token is treated as "not configured": core logs a warning naming what is missing and stores the report without forwarding it, rather than failing the request. |
| v4.0 (unreleased) | `spring.security.oauth2.resourceserver.jwt.jwk-set-uri` and `.issuer-uri` added (EZ-1724) | **Required on any environment with `easy.core.auth-enabled: true`** — core now verifies Keycloak tokens itself instead of trusting an OIDC proxy's `oidc_claim_*` headers. Add both keys pointing at that environment's realm; see the sample for why both and not just `issuer-uri`. The matching change on the webserver is that the API vhost must stop authenticating: `ansible/roles/nginx` templates a plain `proxy_pass`, and a hand-maintained Apache vhost needs its mod_auth_openidc config removed in the same window, since a vhost that still authenticates will 401 cross-origin preflight `OPTIONS` requests. |
