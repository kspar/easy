...
//        }
    ...
    private fun buildTeacherExercises(courseId: String) {
        MainScope().launch {
            val exercisesPromise = fetchEms(
                "/teacher/courses/$courseId/exercises", ReqMethod.GET,
            ...
            val exerciseArray = exercises.exercises
                ...
                .map { ex ->
                    ...
                    exMap["studentsExist"] =
                        ex.completed_count + ex.started_count + ex.ungraded_count + ex.unstarted_count != 0

                    val counts =
                        StudentCounts(ex.completed_count, ex.started_count, ex.ungraded_count, ex.unstarted_count)
    ...
    data class StudentCounts(
        ...
        val unstartedCount: Int
    )

    data class StudentShares(
        val completedShare: Double,
        val startedShare: Double,
        val ungradedShare: Double,
        val unstartedShare: Double
    )
    ...
    private fun calculateStudentShares(counts: StudentCounts): StudentShares {
        ...
        val correctedShares = mapOf(
            "completed" to counts.completedCount,
        ...
        return StudentShares(
            backcorrectedShares["completed"] ?: 0.0,
